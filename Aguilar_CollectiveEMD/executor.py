...
def prepare_for_training(filename):
        
    ###############################
    ## LOADING DATA
    ###############################
    
    (tweets_train, labels_train) = utils.read_datasets()
    postag_train = utils.read_and_sync_postags(tweets_train)
    
    print ('Argument List:', filename)
    global TEST
    # TEST=TEST+filename
    
    TEST_POSTAG              = TEST+filename  + '.postag'
    TEST_PREPROC_URL         = TEST+filename  + '.preproc.url'
    TEST_PREPROC_URL_POSTAG  = TEST+filename  + '.preproc.url.postag'
    ...
    (tweets_test, labels_test) = utils.read_datasets_test(TEST_PREPROC_URL)
    ...
    postag_test = utils.read_and_sync_postags_test(tweets_test,TEST_PREPROC_URL_POSTAG)
    ...
    return tweets_test, labels_test, postag_test, word2index, index2postag, gaze2index, gaze_inputs, char_inputs, word_inputs, mtl_network, fextractor

# def execute(filename):
# def execute(filename, word2index, index2postag, gaze2index, gaze_inputs, char_inputs, word_inputs, mtl_network, fextractor):
def execute(tweets_test, labels_test, postag_test, word2index, index2postag, gaze2index, gaze_inputs, char_inputs, word_inputs, mtl_network, fextractor):
    ...
    sentence_df_dict, tweet_to_sentences_w_annotation = utils.save_predictions(CRF_PREDICTIONS, tweets_test, labels_test, decoded_predictions, network_features)
    
    # getting embeddings for stsdata-- to train phrase embedder-- NOT REQD TO RUN FOR EMD!!!
    # sentence_df_dict, tweet_to_sentences_w_annotation = utils.postprocess_for_sts_dataset(CRF_PREDICTIONS, tweets_test, labels_test, decoded_predictions, network_features)
    # utils.save_predictions_wo_eval(CRF_PREDICTIONS, tweets_test, labels_test, decoded_predictions)
    
    # #WHEN NOT DOING EVALUATION: FOR EFFICIENCY TEST
    # sentence_df_dict, tweet_to_sentences_w_annotation = utils.save_predictions_wo_eval(CRF_PREDICTIONS, tweets_test, labels_test, decoded_predictions, network_features)
