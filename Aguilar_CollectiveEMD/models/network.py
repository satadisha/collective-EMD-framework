...
################################################################################
def build_multitask_bin_cat_network(nb_classes,char_inputs, char_encoded,word_inputs, word_encoded,gaze_inputs, gaze_encoded):
...
def build_multitask_seg_cat_network(nb_classes,char_inputs, char_encoded,word_inputs, word_encoded, gaze_inputs, gaze_encoded):
    network = concatenate([gaze_encoded, char_encoded, word_encoded], name='concat_layer')
    network = Dense(100, activation='relu', name='common_dense_layer')(network)
...
def train_multitask_net_with_split(mtl_model,
    ...
    hist = mtl_model.fit(x_train,
                         ...
                         callbacks=[early_stopping])
    mtl_model.save_weights("saved/My_Custom_Model.h5")
    print("Saved model to disk")
    return hist

################################################################################
def predict(mtl_model, network_inputs, test_samples, index2label, verbose=False):
    # Single-task Network
    network = Model(inputs=network_inputs,
                    outputs=mtl_model.get_layer('cat_output').output,
                    name='cat_model')
    # Predicting test data: batch_size=500
